...
void Paint_NewImage(uint8_t *image,uint16_t Width,uint16_t Height,uint16_t Rotate,uint16_t Color)
{
  ...
  Paint.widthByte = (Source_Bits%4==0)?(Source_Bits/4):(Source_Bits/4+1);
  Paint.heightByte = Gate_Bits;
  Paint.rotate = Rotate;
...
/*******************************************************************
    ...
    接口说明：x      图片x坐标起始位置参数
              ...
              BMP    图片取模数据
    返回值：  无
    函数说明：由于屏幕分辨率为 250x122 实际给参数需要为 250x128此处需要注意
*******************************************************************/
...
void EPD_ShowFourColorPicture(uint16_t x,uint16_t y,uint16_t sizex,uint16_t sizey,const uint8_t BMP[])
{
  ...
  TypefaceNum=sizex*(sizey/4+((sizey%4)?1:0));
  y0=y;
  for(i=0;i<TypefaceNum;i++)
  {
    temp=BMP[j];
