...
// 读取
void W25Q32_Read(uint8_t block, uint8_t sector, uint8_t page, uint8_t *buffer, uint16_t len)
{
    // 1. 先等待不为busy
    W25Q32_WaitNotBusy();
...
// 随机地址读取
void W25Q32_ReadRandom(uint32_t addr, uint8_t *buffer, uint16_t len)
{
    // 1. 先等待不为busy
    W25Q32_WaitNotBusy();
